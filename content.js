...
function waitForButton() {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(document.getElementsByTagName("button"));
        }, 2000);
    });
}
...
function waitForHomeIcon() {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(document.getElementsByClassName("css-175oi2r r-sdzlij r-dnmrzs r-1awozwy r-18u37iz r-1777fci r-xyw6el r-o7ynqc r-6416eg"));
        }, 2000);
    });
}
...
async function replaceHeader(){
    let headers = await waitForLogo();
    if (headers.length > 0) {
        let imgReplace = headers[0].getElementsByTagName("svg");
        if (imgReplace.length > 0) {
...
async function replacePostButton(){
    let tweetButtons = await waitForButton();
    if (tweetButtons.length > 17) {
...
async function replaceHomeIcon(){
    let classBar = await waitForHomeIcon();
...
// Add Mutation Observer with debounce to avoid continuous triggering
let sidebarTimeout;
const observer = new MutationObserver(mutations => {
    ...
    sidebarTimeout = setTimeout(() => {
        replaceHomeIcon();
        replaceHeader();
        observer.disconnect(); // Disconnect after the first successful modification to avoid unnecessary loops
    }, 100);
});
